...
class TestCythonParity:
    def test_hamming_parity(self):
        ...
        assert hamming_similarity(a, b) == pytest.approx(cy_sim(a, b))
    ...
    def test_edit_distance_parity(self):
        ...
        seq_a = ["k", "æ", "t", "s"]
        seq_b = ["b", "ɛ", "d", "z"]
        assert feature_edit_distance(seq_a, seq_b, SAMPLE_FEATURES) == pytest.approx(
            cy_ed(seq_a, seq_b, SAMPLE_FEATURES)
        )
...
class TestPrangeDictionaryScan:
    ...
    def test_prange_matches_sequential(self, ptd_and_feats):
        ...
        seq_results = batch_dictionary_scan(
            source_tokens, source_len, ptd, feats, top_n=10, max_distance=0.50
        )
        par_results = prange_batch_dictionary_scan(
            source_tokens, source_len, ptd, feats, top_n=10, max_distance=0.50, num_threads=2
        )
    ...
    def test_prange_tight_threshold(self, ptd_and_feats):
        ...
        results = prange_batch_dictionary_scan(
            ["b", "æ", "t"], 3, ptd, feats, top_n=10, max_distance=0.01
        )
