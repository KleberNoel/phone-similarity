...
for language in LANGUAGES_TO_TEST:
    try:
        ...
        count = 0
        for word, pronounciations in g2p.pdict.items():
            for p in pronounciations.split():
                try:
                    b_array = bitarray_spec.ipa_to_syllable(clean_phones(p))
                    syllables = []
                    for syllable in b_array:
                        onset = frozenbitarray(syllable.get("onset", bitarray_spec.empty_vector("consonant")))
                        nucleus = frozenbitarray(syllable.get("nucleus", bitarray_spec.empty_vector("vowel")))
        ...
        print(f"'{language}': {{'onset': ({round(metrics.onset_entropy - 0.5, 1)}, {round(metrics.onset_entropy + 0.5, 1)}), 'nucleus': ({round(metrics.nucleus_entropy - 0.5, 1)}, {round(metrics.nucleus_entropy + 0.5, 1)}), 'coda': ({round(metrics.coda_entropy - 0.5, 1)}, {round(metrics.coda_entropy + 0.5, 1)})}},")
