...
def main():
    ...
    print(f"  Memory after specs: current={mem1[0]/1024/1024:.1f}MB, peak={mem1[1]/1024/1024:.1f}MB")
    print()
    ...
    print(f"  Memory after PTDs: current={mem2[0]/1024/1024:.1f}MB, peak={mem2[1]/1024/1024:.1f}MB")
    print()
        ...
        for _ in range(200):
            for i in range(len(token_seqs)):
                for j in range(i + 1, len(token_seqs)):
                    normalised_feature_edit_distance(
                        token_seqs[i], token_seqs[j], source_features
                    )
                    total_comparisons += 1
    ...
    phrases_for_scan = [
        (name, clean_phones(ipa)) for name, ipa in TEST_PHRASES[:3]
    ]
    ...
    for lang in scan_languages:
        ...
        with _timer(f"scan {lang} ({len(ptd)} entries, {len(phrases_for_scan)} phrases)"):
            for _ in range(N_SCAN_ITERATIONS):
                for phrase_key, source_ipa in phrases_for_scan[:1]:
                    ...
                    if _HAS_CYTHON_EXT:
                        ...
                        _c_batch_dictionary_scan(
                            source_tokens, len(source_tokens), ptd, merged,
                            1, 0.50,
                        )
                    else:
                        reverse_dictionary_lookup(
                            source_ipa, SOURCE_LANG, source_spec, source_features,
                            lang, target_specs[lang], target_features[lang], {},
                            pre_tokenized=ptd,
                            top_n=1, max_distance=0.50,
                        )

    mem3 = tracemalloc.get_traced_memory()
    print(f"  Memory after scans: current={mem3[0]/1024/1024:.1f}MB, peak={mem3[1]/1024/1024:.1f}MB")
    print()
    ...
    for cycle in range(3):
        ...
        print(f"  After cycle {cycle+1}: current={mem_now[0]/1024/1024:.1f}MB, peak={mem_now[1]/1024/1024:.1f}MB")
